...
import 'package:aleedz/view/screens/home_chart/weekly_sale_chart_team.dart';
import 'package:aleedz/viewmodel/home_chart_team_viewmodel.dart';
...
class HomeChartTeamView extends ConsumerStatefulWidget {
  const HomeChartTeamView({super.key});

  @override
  ConsumerState<HomeChartTeamView> createState() => _HomeTeamViewState();
}
...
class _HomeTeamViewState extends ConsumerState<HomeChartTeamView> {
  ...
  Future<void> _updateDate(
    DateTime date,
    HomeChartTeamViewModel viewModel,
  ) async {
    setState(() {
      _selectedDate = date;
    });
    ...
    await ref
        .read(homeChartTeamMP.notifier)
        .updateSelectedMonth(monthName, year: date.year.toString());
  }

  Future<void> _changeDateBy(int days, HomeChartTeamViewModel viewModel) async {
    final newDate = _selectedDate.add(Duration(days: days));
    if (_isFutureDate(newDate)) return;
    await _updateDate(newDate, viewModel);
  }

  Future<void> _openDatePicker(HomeChartTeamViewModel viewModel) async {
  ...
  Widget _dateSelector(HomeChartTeamViewModel viewModel) {
  ...
  Future<void> loadUserAndFetchHome() async {
    await ref.read(homeChartTeamMP.notifier).loadDashboard(context);
  }
  ...
  @override
  Widget build(BuildContext context) {
    final viewModel = ref.watch(homeChartTeamMP);
